...
def compress_pickle(input_path, target_size_mb=25):
    ...
    if compressed_size <= target_size_mb:
        ...
        st.success(f"✅ Model compressed successfully: {compressed_size:.2f} MB")
    else:
        os.remove(temp_path)
...
if submitted:
    try:
        ...
        for crop, prob in top3:
            ...
            st.markdown(f"""
                <div style="background-color:#f8fafc;border-radius:10px;padding:15px;margin-bottom:10px;
                            border-left:5px solid #10b981;">
                    <b>{crop}</b> — <span style="color:#059669;">{prob*100:.1f}% Match</span><br>
                    🌾 <b>Expected Yield:</b> {yield_est}<br>
                    💰 <b>Estimated Profit:</b> {profit_est}
                </div>
